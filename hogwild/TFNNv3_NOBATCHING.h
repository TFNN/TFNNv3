...
--------------------------------------------------
    James William Fletcher (github.com/tfnn)
        July 2022 - TFNNv3 (v3.04) No Batching
--------------------------------------------------
    ...
    !!! Batching stripped from this version. !!!

    Cannot share saved networks between versions!
...
#define ERROR_BAD_FUNCTION_PARAMETERS -13
...
**************************************/
    ...
    // output layer error/loss
    float oe[net->num_outputs];
    f32 total_loss = 0.f;
    for(int i = 0; i < net->num_outputs; i++)
    {
        //oe[i] = fabsf(target_outputs[i] - os[i]); // absolute error
        //oe[i] = powf(target_outputs[i] - os[i], 2.f); // squared error
        oe[i] = target_outputs[i] - os[i]; // bidirectional error

        total_loss += fabsf(target_outputs[i] - os[i]); // we return actual loss as a metric
    }

    // early return if total loss is 0
    if(total_loss == 0.f)
    {
        if(outputs != NULL)
            memcpy(outputs, &os, net->num_outputs * sizeof(f32));
...
**************************************/
    ...
    for(int i = 1; i < net->num_layers-1; i++)
    {
        for(int j = 0; j < net->num_layerunits; j++)
        {
            ...
            for(int k = 0; k < net->layer[i][j].weights; k++)
                net->layer[i][j].data[k] += doDropout(net, Optimiser(net, of[i-1][j], ef[i][j], &net->layer[i][j].momentum[k]), dt);
    ...
    {
        for(int j = 0; j < net->layer[net->num_layers-1][0].weights; j++)
            net->layer[net->num_layers-1][i].data[j] += Optimiser(net, of[net->num_layers-2][j], oe[i], &net->layer[net->num_layers-1][i].momentum[j]);

        net->layer[net->num_layers-1][i].bias += Optimiser(net, 1, oe[i], &net->layer[net->num_layers-1][i].bias_momentum);
    }
